        ...
        for class_name, sampled_num in zip(sampled_groups, sample_num_per_class):
            if sampled_num > 0:
                if self._use_group_sampling:
                    ...
                    # get a list of valid objects to be augmented
                    # for a given class "class_name"
                    # both bounding box check and also frustum check is done
        ...
        if len(sampled) > 0:
            ...
            for i, info in enumerate(sampled):
                try:
                    ...
                    # only for testing : please change it later
                    # s_points = s_points.reshape(-1, 5)

                    s_points = s_points.reshape(-1, 6 + 3)[:, selected_feature > 0.]
                ...
                if revise_calib and calib is not None:
                    ...
                    # Originally there was a cam but now there is no -> the projection point cam_id is set to infinity
