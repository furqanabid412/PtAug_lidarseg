...
class NuScenesDataset(PointCloudDataset):
    ...
    def get_sensor_data(self, idx):
        ...
        self.test_plot = True
        import os
        path = 'E:\work\pointcloud\TestCodes\PtAugsimplified\cases/01'
        ...
        for i in range(6):
            if self.test_plot:
                im_cv = img[i][:, :, ::-1]
                # plt.imshow(img[i][:,:,::-1])
                # plt.title(str(self.cam_name[i]))
                # plt.show()
                # filepath = os.path.join(str(path), 'seq{}_{}_orig_{}.jpg'.format(EQ,i, self.cam_name[i]))
                # cv2.imwrite(filepath, cv2.cvtColor(im_cv, cv2.COLOR_BGR2RGB))
        ...
        for i in range(6):
            if self.test_plot:
                im_cv = data['img'][i][:, :, ::-1]
                # plt.imshow(data['img'][i][:, :, ::-1])
                # plt.title(str(self.cam_name[i]))
                # plt.show()
                # filepath = os.path.join(str(path), 'seq{}_{}_aug_{}.jpg'.format(EQ,i, self.cam_name[i]))
                # cv2.imwrite(filepath, cv2.cvtColor(im_cv, cv2.COLOR_BGR2RGB))
