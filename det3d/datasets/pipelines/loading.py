...
def visualize_(res,pts_uv,mask,cam_id):
    # visualization test
    import cv2
    img = res["img"][cam_id]
...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
            ...
            if res['mode']=='train':
                lidarseg_labels = np.expand_dims(load_bin_file(Path(info["ref_lidarseg"]["filename"]), type='lidarseg'),
                                             axis=1)
            else:
                lidarseg_labels = np.full((len(points[:,0]),1),-1,dtype=np.int32)
            points = np.concatenate([points, lidarseg_labels], axis=1)
            ...
            if self.use_img:
                ...
                for cam_id, cam_sensor in enumerate(cam_name):
                    ...
                    # to visualize
                    # visualize_(res, pts_uv, mask, cam_id)
